...
)
{
	...
	Sections = reinterpret_cast<EFI_IMAGE_SECTION_HEADER*>(reinterpret_cast<PUINT8>(&Nt->OptionalHeader) + Nt->FileHeader.SizeOfOptionalHeader);
...
)
{
	...
	UINT16 SecCount = 0;
	EFI_STATUS Status = GetNtHeaders64(ImageBase, NtHeaders, Sec, SecCount);
...
)
{
	...
	for (UINT64 i = Start; i >= Min; i -= PAGE_SIZE)
	{
		...
		EFI_IMAGE_SECTION_HEADER* Sec = nullptr;
		UINT16 SecCount = 0;
		EFI_STATUS Status = GetNtHeaders64(Base, NtHeaders, Sec, SecCount);
